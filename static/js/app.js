...
function setEmptyToursLists() {
    const lists = ['toursListOswald', 'toursListAlexandre', 'toursListDamien'];
    lists.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.innerHTML = '<p class="empty-history">Keine Touren</p>';
    });
}
...
function displayTours(tours) {
    ...
    if (!tours || tours.length === 0) {
        Object.values(lists).forEach(el => { if (el) el.innerHTML = '<p class="empty-history">Keine Touren</p>'; });
        return;
    }
    ...
    tours.forEach(t => {
        ...
        else toursByUser.Oswald.push(t);
    });
    ...
    ['Oswald', 'Alexandre', 'Damien'].forEach(user => {
        ...
        tourList.forEach((tour, displayIndex) => {
            ...
            const photoPreviewHtml = hasPhotos
                ? `<div class="tour-photo-preview" title="Fotos anzeigen"><img src="${escapeHtml(firstPhotoUrl)}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';"><span class="tour-photo-icon-fallback" style="display:none">📸</span></div>`
                : '';
            ...
            <div class="tour-mobile-row">
                <div class="tour-mobile-calendar"><div class="tour-date-icon"><span class="tour-cal-day">${parsed.day}</span><span class="tour-cal-month">${parsed.month}</span></div></div>
                <div class="tour-mobile-center"><span class="tour-mobile-km">${formatDistance(tour.Km || 0)}</span>${photoThumbHtml}</div>
            ...
            <div class="tour-field tour-desktop-only">
                ...
                <div class="tour-datum-row">
                    <div class="tour-date-icon" title="${escapeHtml(tour.Date || '')}"><span class="tour-cal-day">${parsed.day}</span><span class="tour-cal-month">${parsed.month}</span></div>
                    <span>${tour.Date || ''}</span>
                </div>
                <span class="tour-user-pill tour-user-pill-${userKey}" title="${user}">${ { Oswald: '🌳', Alexandre: '🌴', Damien: '⚡' }[user] } ${user}</span>
                ${tour.Wetter && String(tour.Wetter).trim() && tour.Wetter !== 'N/A' ? `<span class="tour-wetter">🌤️ ${escapeHtml(String(tour.Wetter).trim())}</span>` : ''}
            </div>
            <div class="tour-field tour-desktop-only"><strong>Start</strong><span>${tour.Start || ''}</span></div>
            <div class="tour-field tour-desktop-only"><strong>Ziel</strong><span>${tour.Ziel || ''}</span></div>
            <div class="tour-field tour-desktop-only"><strong>Km</strong><span>${formatDistance(tour.Km || 0)}</span></div>
            ${tour.Etape && tour.Etape !== 'NaN' && tour.Etape !== 'nan' && tour.Etape !== 'N/A' ? `<div class="tour-field tour-desktop-only"><strong>Etape</strong><span>${tour.Etape}</span></div>` : ''}
            ${tour.Bemerkungen && String(tour.Bemerkungen).trim() ? `<div class="tour-remark tour-desktop-only">${escapeHtml(String(tour.Bemerkungen).trim())}</div>` : ''}
...
function updateStats(data) {
    ...
    users.forEach((name, i) => {
        ...
        const ids = ['Aujourdhui', 'Semaine', 'Mois', 'Annee', 'Total'];
        const keys2 = ['total_aujourdhui', 'total_semaine', 'total_mois', 'total_annee', 'total_global'];
        ids.forEach((suffix, j) => {
            const el = document.getElementById(`stat${name}${suffix}`);
            if (el) el.textContent = formatDistance(stats[keys2[j]] || 0);
        });
    });
}
...
function updateChallenge(challenge) {
    ...
    if (msgEl) msgEl.textContent = challenge.leader === 'Unentschieden'
        ? 'Unentschieden! Keine Touren eingetragen oder gleiche Strecke.'
        : `${challenge.leader} führt mit ${formatDistance(challenge.difference || 0)} Vorsprung!`;
    const pct = (km) => Math.min(100, (km / TOUR_DU_MONDE_KM) * 100);
    const markerO = document.getElementById('challengeMarkerOswald');
    const markerA = document.getElementById('challengeMarkerAlexandre');
    const markerD = document.getElementById('challengeMarkerDamien');
    if (markerO) markerO.style.left = `${pct(challenge.total_oswald || 0)}%`;
    if (markerA) markerA.style.left = `${pct(challenge.total_alexandre || 0)}%`;
    if (markerD) markerD.style.left = `${pct(challenge.total_damien || 0)}%`;
}
...
function updateHistoryComparativeBar(challenge) {
    ...
    let wO = 0, wA = 0, wD = 0;
    if (total > 0) {
        const filled = Math.min(100, (total / TOUR_DU_MONDE_KM) * 100);
        wO = (oswald / total) * filled;
        wA = (alexandre / total) * filled;
        wD = (damien / total) * filled;
    }
    const barO = document.getElementById('historyBarOswald');
    const barA = document.getElementById('historyBarAlexandre');
    const barD = document.getElementById('historyBarDamien');
    if (barO) barO.style.width = `${wO}%`;
    if (barA) barA.style.width = `${wA}%`;
    if (barD) barD.style.width = `${wD}%`;
}
...
function updateProgression(data) {
    ['Oswald', 'Alexandre', 'Damien'].forEach(name => {
        ...
        const elMsg = document.getElementById(`globeMessage${name}`);
        const elVille = document.getElementById(`villeActuelle${name}`);
        const elProchaine = document.getElementById(`prochaineVille${name}`);
        const elLabel = document.getElementById(`progressBarLabel${name}`);
        const elKm = document.getElementById(`kmRestants${name}`);
        const elFill = document.getElementById(`progressFill${name}`);
        const elText = document.getElementById(`progressText${name}`);
    ...
    const progO = data.progression_oswald || {};
    const progA = data.progression_alexandre || {};
    const progD = data.progression_damien || {};
    const emailLink = document.getElementById('emailLink');
    if (emailLink) emailLink.href = `mailto:cogi003@gmail.com?subject=${encodeURIComponent("Opa's Bicycle Update")}&body=${encodeURIComponent(
...
function renderGarageCards(bikes) {
    ...
    gallery.innerHTML = bikes.map(b => {
        ...
        const soon = isSoon(b.date_prochain_entretien);
        const overdue = isOverdue(b.date_prochain_entretien);
        ...
        <div class="garage-card-item" data-id="${b.id}" style="--accent: ${color}">
            <button type="button" class="garage-card-edit-btn" data-id="${b.id}" data-name="${escapeHtml(b.nom_velo || '').replace(/"/g, '&quot;')}" data-user="${b.utilisateur || 'Oswald'}" data-km="${b.km_actuel || 0}" data-date="${b.date_prochain_entretien || ''}" title="Bearbeiten"><i data-lucide="pencil" aria-hidden="true"></i></button>
            <div class="garage-card-image-wrap">
                <div class="garage-card-image" style="background-image: url('${imgUrl || ''}')">${!imgUrl ? '<span class="garage-card-placeholder">🚲</span>' : ''}</div>
            ...
            <div class="garage-card-info">
                ...
                <p class="garage-card-date ${alertClass}">Nächstes Service: ${fmtDate(b.date_prochain_entretien)}${overdue ? ' (Überfällig!)' : ''}${soon && !overdue ? ' (Bald)' : ''}</p>
                <div class="garage-card-actions">
                    ${b.url_facture ? `<button type="button" class="garage-btn-facture" data-url="${escapeHtml(b.url_facture)}">Facture</button>` : `<label class="garage-btn-add-facture"><input type="file" accept="image/*,.pdf" data-id="${b.id}" style="display:none">+ Facture</label>`}
                </div>
            </div>
        </div>`;
    }).join('');
    ...
    gallery.querySelectorAll('.garage-km-inc').forEach(btn => {
        btn.addEventListener('click', () => incrementBikeKm(parseInt(btn.dataset.id, 10), parseFloat(btn.dataset.delta)));
    });
    ...
    gallery.querySelectorAll('.garage-btn-add-facture input').forEach(inp => {
        inp.addEventListener('change', (e) => { if (e.target.files && e.target.files[0]) uploadBikeFacture(parseInt(inp.dataset.id), e.target.files[0]); });
    });
    if (typeof lucide !== 'undefined') lucide.createIcons();
}
...
async function handleAddBikeSubmit(e) {
    ...
    try {
        if (isEdit) {
            ...
            if (factureInput && factureInput.files && factureInput.files[0]) {
                const fd = new FormData();
...
async function incrementBikeKm(bikeId, delta) {
    try {
        ...
        const bikes = dataRes.bikes || [];
        const b = bikes.find(x => x.id === bikeId);
        if (!b) return;
        const newKm = (b.km_actuel || 0) + delta;
        const putRes = await fetch(`${API_BASE}/api/entretien/${bikeId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ km_actuel: newKm })
        });
        const putData = await putRes.json();
        if (putData.success) { showToast(`+${delta} km`, 'success'); incrementAppBadge(); await loadGarage(); }
        else throw new Error(putData.error);
...
async function syncBikeKm(bikeId, user) {
    ...
    try {
        ...
        const putData = await putRes.json();
        if (putData.success) { showToast(`+${formatNumber(kmFromTours, 0)} km (Touren)`, 'success'); incrementAppBadge(); await loadGarage(); }
        else throw new Error(putData.error);
    } catch (e) { showToast('Fehler', 'error'); }
}
...
async function uploadBikeFacture(bikeId, file) {
    try {
        ...
        const data = await res.json();
        if (data.success) { showToast('Facture hinzugefügt', 'success'); incrementAppBadge(); await loadGarage(); }
...
function openFactureModal(url) {
    ...
    if (img && m && url) { img.src = url; m.style.display = 'flex'; m.classList.add('modal-open'); m.setAttribute('aria-hidden', 'false'); }
}
...
function openTourModal(tour) {
    ...
    const modalWetter = document.getElementById('modalWetter');
    ...
    if (startTime && zielTime) {
        ...
        if (mins > 0) {
            ...
            if (km > 0) speedStr = `${(km / (mins / 60)).toFixed(1).replace('.', ',')} km/h`;
        }
    }
    if (modalTime) modalTime.textContent = timeStr;
    if (modalSpeed) modalSpeed.textContent = speedStr;
    if (modalWetter) modalWetter.textContent = tour.Wetter && tour.Wetter !== 'N/A' ? tour.Wetter : '—';

    const bemerkWrap = document.getElementById('modalBemerkungenWrap');
    const bemerkungen = document.getElementById('modalBemerkungen');
    if (tour.Bemerkungen && String(tour.Bemerkungen).trim()) {
        if (bemerkWrap) bemerkWrap.style.display = 'block';
        if (bemerkungen) bemerkungen.textContent = tour.Bemerkungen.trim();
    } else {
        if (bemerkWrap) bemerkWrap.style.display = 'none';
    }
    ...
    if (photos.length > 0 && photosGrid) {
        if (photosWrap) photosWrap.style.display = 'block';
        photosGrid.innerHTML = photos.map(url => `<a href="${escapeHtml(url)}" target="_blank" rel="noopener" class="modal-photo-item"><img src="${escapeHtml(url)}" alt="Photo"></a>`).join('');
    } else {
    ...
    if (photoBtn && photoInput && !photoBtn.disabled) {
        ...
        photoInput.onchange = (e) => {
            ...
            if (file.size > 5 * 1024 * 1024) { showToast('Image trop lourde (max 5 Mo)', 'error'); photoInput.value = ''; return; }
...
async function uploadTourPhoto(tourId, file) {
    const photoBtn = document.getElementById('modalPhotoBtn');
    if (photoBtn) { photoBtn.disabled = true; photoBtn.textContent = '⏳ Envoi...'; }
    try {
        const fd = new FormData();
        fd.append('photo', file);
        const res = await fetch(`${API_BASE}/api/tours/${tourId}/photos`, { method: 'POST', body: fd });
        const data = await res.json();
        if (data.success) {
            showToast('Photo ajoutée ! 📸', 'success');
            await loadEntries();
            const updated = toursData.tours && toursData.tours.find(t => t._index === tourId);
        ...
        if (photoBtn) { photoBtn.disabled = false; photoBtn.textContent = '📸 Ajouter des photos'; }
    }
}
...
function printTourDetail() {
    ...
    const content = document.querySelector('.modal-glass');
    if (!content) return;
...
// =============================================================================
// FORMULAIRE TOUR, SUPPRESSION, TOAST
// =============================================================================
...
async function handleFormSubmit(e) {
    ...
    const formData = {
        ...
        depart: document.getElementById('depart')?.value || 'Kettenis',
        etape: document.getElementById('etape')?.value || '',
        arrivee: document.getElementById('arrivee')?.value || 'Kettenis',
        distance: parseFloat(document.getElementById('distance')?.value) || 0,
        heure_depart: document.getElementById('heure_depart')?.value || '10:00',
        heure_etape: document.getElementById('heure_etape')?.value || '11:30',
        heure_arrivee: document.getElementById('heure_arrivee')?.value || '12:30',
    ...
    try {
        ...
        if (data.success) {
            ...
            const heure_etape = document.getElementById('heure_etape');
            const heure_arrivee = document.getElementById('heure_arrivee');
            const utilisateur = document.getElementById('utilisateur');
            if (depart) depart.value = 'Kettenis';
            if (arrivee) arrivee.value = 'Kettenis';
            if (heure_depart) heure_depart.value = '10:00';
            if (heure_etape) heure_etape.value = '11:30';
            if (heure_arrivee) heure_arrivee.value = '12:30';
            if (utilisateur) utilisateur.value = 'Oswald';
            await loadEntries();
            celebrate();
        ...
        if (submitBtn) { submitBtn.disabled = false; const span = submitBtn.querySelector('span'); if (span) span.textContent = 'Tour speichern'; }
        if (meteoLoading) meteoLoading.style.display = 'none';
    }
}
...
async function deleteTour(index) {
    ...
    try {
        const res = await fetch(`${API_BASE}/api/tours/${index}`, { method: 'DELETE' });
        const data = await res.json();
        if (data.success) { showToast('Tour gelöscht', 'success'); await loadEntries(); }
...
function setStoredBadgeCount(n) {
    try { localStorage.setItem(BADGE_STORAGE_KEY, String(Math.max(0, n))); } catch (_) {}
}
    ...
    (async () => {
        ...
        try {
            ...
            if (verRes.ok) {
                ...
                if (lastKnownVersion !== null && cur !== null && cur !== lastKnownVersion) hasNew = true;
            }
        } catch (_) {}
        if (!hasNew) {
            try {
...
function initLiveNotification() {
    ...
    if (btn) btn.addEventListener('click', () => { clearAppBadge(); window.location.reload(true); });
}
...
function createConfetti() {
    ...
    ], { duration: 2000 + Math.random() * 1000, easing: 'cubic-bezier(0.5, 0, 0.5, 1)' }).onfinish = () => confetti.remove();
}
